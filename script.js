...
var tombolPengingat = document.getElementById('tombolPengingat');

function goyangkanIkonBel() {
    if (!tombolPengingat) return;
    var ikon = tombolPengingat.querySelector('.ikon-bel');
    if (!ikon) return;
    ikon.classList.remove('berdering');
    void ikon.offsetWidth; // trik biar animasinya bisa ke-restart
    ikon.classList.add('berdering');
}

function formatTanggalPengingat(tanggal) {
    var namaBulan = [
        'Januari', 'Februari', 'Maret', 'April', 'Mei', 'Juni',
        'Juli', 'Agustus', 'September', 'Oktober', 'November', 'Desember'
    ];
    var dua = function (n) { return n < 10 ? '0' + n : '' + n; };
    return tanggal.getDate() + ' ' + namaBulan[tanggal.getMonth()] + ' ' + tanggal.getFullYear() +
        ', pukul ' + dua(tanggal.getHours()) + '.' + dua(tanggal.getMinutes()) + ' WIB';
}

// Bikin sejumlah kertas konfeti kecil warna-warni buat hiasan pop-up
function buatKonfetiPopup() {
    var warna = ['#FFCF4F', '#626F44', '#A6B465', '#ED861E'];
    var html = '';
    for (var i = 0; i < 16; i++) {
        var kiri = Math.round(Math.random() * 100);
        var delay = (Math.random() * 1.6).toFixed(2);
        var durasi = (2.2 + Math.random() * 1.8).toFixed(2);
        var warnaTerpilih = warna[i % warna.length];
        html +=
            '<span class="konfeti-popup" style="' +
            'left:' + kiri + '%;' +
            'background-color:' + warnaTerpilih + ';' +
...
function tampilkanPopupPengingat() {
    var popupLama = document.querySelector('.popup-pengingat-overlay');
    if (popupLama) popupLama.remove();

    // Samain sama waktuTarget di "Bagian countdown" di atas (29 Agustus
    // 2026, jam 5 pagi) - ditulis ulang di sini biar section ini berdiri
    // sendiri tanpa gantung ke variabel di section lain
    var tanggalTarget = new Date(2026, 7, 29, 5, 0, 0);
    var teksTanggal = formatTanggalPengingat(tanggalTarget);

    var overlay = document.createElement('div');
    overlay.className = 'popup-pengingat-overlay';
    overlay.innerHTML =
        '<div class="popup-pengingat" role="dialog" aria-modal="true" aria-labelledby="popupPengingatJudul">' +
            '<div class="popup-pengingat-konfeti">' + buatKonfetiPopup() + '</div>' +
            '<button type="button" class="popup-pengingat-tutup" aria-label="Tutup pop-up">&times;</button>' +
            '<div class="popup-pengingat-lonceng">🔔</div>' +
            '<h3 id="popupPengingatJudul">Jangan Lupa Mendaftar!</h3>' +
            '<p>Pendaftaran Staff Muda UAKI 2026 dibuka pada<br><strong>' + teksTanggal + '</strong></p>' +
            '<button type="button" class="popup-pengingat-oke">Oke, Siap!</button>' +
        '</div>';

    document.body.appendChild(overlay);

    // requestAnimationFrame biar transition CSS-nya sempat ke-trigger
    ...
    overlay.addEventListener('click', function (e) {
        if (e.target === overlay) tutupPopup(); // klik area gelap luar kartu
    });
    overlay.querySelector('.popup-pengingat-tutup').addEventListener('click', tutupPopup);
    overlay.querySelector('.popup-pengingat-oke').addEventListener('click', tutupPopup);
    document.addEventListener('keydown', escHandler);
}

if (tombolPengingat) {
    tombolPengingat.addEventListener('click', function () {
        goyangkanIkonBel();
        tampilkanPopupPengingat();
    });
}
